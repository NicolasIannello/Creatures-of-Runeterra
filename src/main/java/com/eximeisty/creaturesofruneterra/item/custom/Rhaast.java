...
public class Rhaast extends SwordItem implements IAnimatable {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerentity, Hand handIn) {
        ...
        if(stack.getDamage()==0 && playerentity.getItemStackFromSlot(EquipmentSlotType.HEAD).getItem() instanceof ArmorItem ? ((ArmorItem)playerentity.getItemStackFromSlot(EquipmentSlotType.HEAD).getItem()).getArmorMaterial()!= ModArmorMaterial.DARKIN : playerentity.getItemStackFromSlot(EquipmentSlotType.HEAD).isEmpty()){
            ...
            for (int i = 0; i < 4; i++) {
                ItemStack helmet = playerentity.getItemStackFromSlot(es[i]).copy();
                ...
                playerentity.setItemStackToSlot(es[i], Rhelmet);
            }
