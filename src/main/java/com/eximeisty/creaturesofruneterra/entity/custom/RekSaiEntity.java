...
public class RekSaiEntity extends PathfinderMob implements IAnimatable {
    ...
    private static final AnimationBuilder WALK_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.walk", ILoopType.EDefaultLoopTypes.LOOP);
    private static final AnimationBuilder RUN_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.run", ILoopType.EDefaultLoopTypes.LOOP);
    private static final AnimationBuilder IDLE_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.idle", ILoopType.EDefaultLoopTypes.LOOP);
    private static final AnimationBuilder IDLE2_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.idle2", ILoopType.EDefaultLoopTypes.LOOP);
    private static final AnimationBuilder ATTACK1_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.attack1", ILoopType.EDefaultLoopTypes.PLAY_ONCE);
    private static final AnimationBuilder SPIN_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.spin", ILoopType.EDefaultLoopTypes.PLAY_ONCE);
    private static final AnimationBuilder ATTACK12_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.attack1", ILoopType.EDefaultLoopTypes.PLAY_ONCE).addAnimation("animation.Reksai.attack2", ILoopType.EDefaultLoopTypes.PLAY_ONCE);
    private static final AnimationBuilder SPIN2_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.spin", ILoopType.EDefaultLoopTypes.PLAY_ONCE).addAnimation("animation.Reksai.spin", ILoopType.EDefaultLoopTypes.PLAY_ONCE);
    private static final AnimationBuilder SLAM_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.slam", ILoopType.EDefaultLoopTypes.PLAY_ONCE);
    private static final AnimationBuilder RDOWN_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.RDown", ILoopType.EDefaultLoopTypes.LOOP);
    private static final AnimationBuilder RTRANS_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.RTrans", ILoopType.EDefaultLoopTypes.PLAY_ONCE);
    private static final AnimationBuilder R2_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.R2", ILoopType.EDefaultLoopTypes.LOOP);
    private static final AnimationBuilder BURROW_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.burrow", ILoopType.EDefaultLoopTypes.PLAY_ONCE).addAnimation("animation.Reksai.charge", ILoopType.EDefaultLoopTypes.LOOP);
    private static final AnimationBuilder CHARGE_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.charge", ILoopType.EDefaultLoopTypes.LOOP);
    private static final AnimationBuilder SALIR_ANIM = new AnimationBuilder().addAnimation("animation.Reksai.salir", ILoopType.EDefaultLoopTypes.PLAY_ONCE);
    ...
    public RekSaiEntity(EntityType<? extends PathfinderMob> type, Level worldIn) {
        super(type, worldIn);
        head=new CoRPartEntity(ModEntityTypes.WIVHIV.get(),this.level);
        body=new CoRPartEntity(ModEntityTypes.WVIHV.get(),this.level);
        leg=new CoRPartEntity(ModEntityTypes.WVIIHVI.get(),this.level);
        tail=new CoRPartEntity(ModEntityTypes.WIIIHIII.get(),this.level);
        tail2=new CoRPartEntity(ModEntityTypes.WIVHIV.get(),this.level);
    }
    ...
    public <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if(entityData.get(STATE)==0 /*|| event.getController().getAnimationState()== AnimationState.Stopped*/){
            if (event.isMoving()) {
    ...
    public void tick() {
        ...
        if(!spawnAnim){
            grabTicks++;
            if(grabTicks==3) this.level.playSound(null, this.getOnPos(), ModSoundEvents.REKSAI_APP.get(), SoundSource.HOSTILE, 3, 1);
            if(grabTicks==35){
                grabTicks=1.0D;
        ...
        if(this.getHealth()<this.getMaxHealth()/2 && entityData.get(RUN)==0){
            velocidad=0.6;
            this.level.playSound(null, this.getOnPos(), ModSoundEvents.REKSAI_ANGRY.get(), SoundSource.HOSTILE, 3, 1);
            this.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(velocidad);
            entityData.set(RUN, 1);
        }
        if(this.getHealth()<=1 || deathTicks>0){
            deathTicks++;
            if(deathTicks==3) this.level.playSound(null, this.getOnPos(), ModSoundEvents.REKSAI_ESCAPE.get(), SoundSource.HOSTILE, 3, 1);
...
//        }
        ...
        public void checkAndPerformAttack(LivingEntity enemy, double distToEnemySqr) {
            ...
            if(distToEnemySqr<100 && entityData.get(STATE)==0 && this.attacker.isOnGround() && throwcd<=0){
            ...
            if(distToEnemySqr<100 && entityData.get(STATE)==0 && this.attacker.isOnGround() && cd<=0){
                //lastAttack=0;
            ...
            if(distToEnemySqr<400 && entityData.get(STATE)==0 && this.attacker.isOnGround() && slam<=0){
                //lastAttack=0;
            ...
            if(distToEnemySqr>550 && entityData.get(STATE)==0 && charge<=0 && this.attacker.isOnGround()){
                entityData.set(STATE, 3);
                this.attacker.getAttribute(Attributes.MOVEMENT_SPEED).setBaseValue(0);
                return;
            }
            if(entityData.get(STATE)==3){
                ...
                if(ticks==30/2){
                    ...
                    if(leap==true){
                        ...
                        this.attacker.level.playSound(null, this.attacker.blockPosition(), ModSoundEvents.REKSAI_JUMP.get(), SoundSource.HOSTILE, 3, 1);
            ...
            if(entityData.get(STATE)==2 || entityData.get(STATE)==1 || entityData.get(STATE)==6){
                ...
                if(distToEnemySqr<30 && grab==false){
                    grab=true;
                    enemy.hurt(DamageSource.mobAttack(this.attacker), this.attacker.damage*20);
                    enemy.stopRiding();
                ...
                if((this.attacker.isOnGround() || this.attacker.isInWater()) && ticks>1){
                    if(grab){
                        ...
                        enemy.hurt(DamageSource.mobAttack(this.attacker), this.attacker.damage*(jumpDamage+15));
                        lastHit=0;
                    }
                    grab=false;
                    ticks=0;
